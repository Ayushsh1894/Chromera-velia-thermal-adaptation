...
strain CCMP2878) and **OTIK/OTI** (One Tree Island, tropical, southern Great
Barrier Reef) — were grown at **20 °C** and **26 °C**, three biological
replicates per condition (12 RNA-seq libraries total). The analysis compares the
two strains at each temperature (*strain difference*), each strain's response to
temperature (*thermal response*), and the core/pan transcriptome.
...
```
.
├── code/                         R analysis scripts (run in numbered order)
...
└── climate_data_plos/            in-situ sea-surface-temperature data for the
...
R (≥ 4.2) with Bioconductor. Scripts are numbered in dependency order; run from
the folder containing the two input files (or set `project_dir` at the top of
each script).
...
| `03_strain_comparison.R` | OTIK-vs-SH at each temperature (per-temperature subsets); Venn; log2FC(26 °C)-vs-log2FC(20 °C) scatter. |
...
- Reference levels: strain = SH, temperature = T20.
  - Strain contrasts: positive log2FC = higher in OTIK.
  - Thermal contrasts: positive log2FC = up at 26 °C.

**Two normalisations, two purposes.** Presence/absence (script 01) uses **edgeR
CPM**; all differential expression (scripts 02–05) uses **DESeq2
median-of-ratios** normalisation. These answer different questions and are not
interchangeable.

**Within-strain thermal contrasts** are computed on per-strain subsets (each
`~ temperature`) rather than from an interaction term on the full model, to
avoid model-fitting issues arising from the very large strain effect.
...
| Strain DEGs at 20 °C | 8,065 (5,284 OTIK-higher / 2,781 SH-higher) |
| Strain DEGs at 26 °C | 8,253 (6,113 OTIK-higher / 2,140 SH-higher) |
| Constitutive strain DEGs (both temperatures) | 6,455 |
| SH thermal DEGs (T26 vs T20) | 4,511 |
| OTIK thermal DEGs (T26 vs T20) | 4,089 |
...
Gene lists exported by the scripts were submitted to **CryptoDB**
(https://cryptodb.org) for functional annotation and Gene Ontology enrichment.
Each workbook contains three sheets with an identical structure:
...
**Strain Difference** — genes differing between strains at a given temperature
(from `04_strain_directional_categories.R`):
...
**Thermal response** — genes responding to temperature within a strain
(from the thermal contrasts and `05_thermal_concordance.R`):
...
## `climate_data_plos/` — sea-surface temperature

In-situ sea-surface-temperature records for the two isolation sites, used to
place the experimental temperatures (20 °C and 26 °C) in the context of each
strain's native thermal environment (temperate Sydney vs the tropical Great
Barrier Reef). Source and processing are described in the Methods.
...
**EnhancedVolcano**, **ggplot2**, **dplyr**, **tibble**, **ggvenn**, **UpSetR**,
**ggrepel**, **pheatmap**, **RColorBrewer**.

## Citation

If you use this code or data, please cite the article (details on publication)
and this repository.
